...
def model():
    ...
    glm_poisson = smf.glm(formula='frecuencia ~ estado + clave_marca + año', data=data_final, family=sm.families.Poisson(link=sm.families.links.Log()))

    # glm_gamma = smf.glm(formula='intensidad ~ estado + clave_marca + año + tipo_poliza', data=data_final, family=sm.families.Poisson(link=sm.families.links.Log()))
    # Ajustar modelo
